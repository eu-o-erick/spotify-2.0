...
export default function ItemAlbumComponent({
  ...
  return (
    <div className="select-none flex flex-col px-3 pt-3 pb-5 rounded-md hover:bg-[#ffffff05] transition-all album-item-container">
      <Link
        href={{
          pathname: `/album`,
...
export default function ItemAlbumComponent({
  ...
  return (
    <div className="select-none flex flex-col px-3 pt-3 pb-5 rounded-md hover:bg-[#ffffff05] transition-all group">
      <Link
        href={{
          pathname: `/album`,
          query: { id: data.uri.split(":")[2] },
      ...
      >
        <ImageLoader
          alt={`cover '${data.name}' album`}
          src={data.coverArt?.sources?.[0]?.url ?? "/no-image.webp"}
          className="rounded-[4px] group-hover:shadow-none max-md:rounded-sm"
        />

        <p className="mt-2 line-clamp-2 max-md:text-[14px] text-zinc-300 group-hover:text-white group-hover:underline transition-all">
          {data.name}
        </p>
      </Link>
